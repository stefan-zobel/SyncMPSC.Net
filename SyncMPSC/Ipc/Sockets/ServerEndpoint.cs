...
{
    private static readonly ILogger<ServerEndpoint> LOGGER = LogManager.GetLogger<ServerEndpoint>();
    ...
    internal void Start()
    {
        ...
        while (Volatile.Read(ref _isListening) == 1)
        {
    ...
    private void HandleClient(TcpClient client)
    {
        ...
        using (NetworkStream stream = client.GetStream())
        {
            ...
            {
                byte[]? frame;
                while ((frame = FrameDecoder.NextFrame(stream, _frameDelimiter)) != null)
                {
